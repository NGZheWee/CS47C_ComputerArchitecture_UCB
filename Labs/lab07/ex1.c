...
long long int sum_simd_unrolled(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        /* Copy your sum_simd() implementation here, and unroll it */
         // Unrolling the SIMD loop 4 times
        __m128i sum_vec = _mm_setzero_si128();
        ...
        for (unsigned int i = 0; i < NUM_ELEMS / 16 * 16; i += 16) {
            ...
            __m128i cmp1 = _mm_cmpgt_epi32(val1, _127);
            __m128i cmp2 = _mm_cmpgt_epi32(val2, _127);
            __m128i cmp3 = _mm_cmpgt_epi32(val3, _127);
            __m128i cmp4 = _mm_cmpgt_epi32(val4, _127);

            val1 = _mm_and_si128(val1, cmp1);
            val2 = _mm_and_si128(val2, cmp2);
            val3 = _mm_and_si128(val3, cmp3);
            val4 = _mm_and_si128(val4, cmp4);

            sum_vec = _mm_add_epi32(sum_vec, val1);
            sum_vec = _mm_add_epi32(sum_vec, val2);
            sum_vec = _mm_add_epi32(sum_vec, val3);
            sum_vec = _mm_add_epi32(sum_vec, val4);
        }
        ...
        for (unsigned int i = NUM_ELEMS / 16 * 16; i < NUM_ELEMS; i++) {
            if (vals[i] >= 128) {
                result += vals[i];
                //sum_vec = _mm_add_epi32(sum_vec, _mm_set1_epi32(vals[i]));
            }
        }

        // Combine the values in sum_vec
        int temp[4];
        _mm_storeu_si128((__m128i *)temp, sum_vec);
        result += temp[0] + temp[1] + temp[2] + temp[3];
