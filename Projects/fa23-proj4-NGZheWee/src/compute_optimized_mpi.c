...
int convolve(matrix_t *a_matrix, matrix_t *b_matrix, matrix_t **output_matrix) {
  ...
  *output_matrix = (matrix_t *)malloc(sizeof(matrix_t));
  ...
  int32_t *a_data = (int32_t *)malloc(sizeof(int32_t) * ksize_align16);
  memset(a_data, 0, sizeof(int32_t) * ksize_align16);
  ...
  if (bsize > 8) {
    ...
    for (int k = 0; k < out_cols * out_rows; k++) {
      ...
      for (int l = 0; l < b_matrix->rows; l++) {
        memcpy(a_data + l * b_matrix->cols, a_matrix->data + (i + l) * a_matrix->cols + j, sizeof(int32_t) * b_matrix->cols);
      }
      // Store the sum in the output matrix
      (*output_matrix)->data[k] = conv1d_align16(a_data, b_data, ksize_align16);
    }
  } else if (bsize > 4) {
    // Loop over each element in the output matrix
    for (int k = 0; k < out_cols * out_rows; k++) {
      ...
      // Store the sum in the output matrix
      (*output_matrix)->data[k] = conv1d_w8(a_data, b_data);
    }
  } else if (bsize > 1) {
    // Loop over each element in the output matrix
    for (int k = 0; k < out_cols * out_rows; k++) {
      ...
      for (int k = 0; k < b_matrix->rows; k++) {
        memcpy(a_data + k * b_matrix->cols, a_matrix->data + (i + k) * a_matrix->cols + j, sizeof(int32_t) * b_matrix->cols);
      }
      // Store the sum in the output matrix
      (*output_matrix)->data[k] = conv1d_w4(a_data, b_data);
    }
  } else {
    if (asize >= 8) {
